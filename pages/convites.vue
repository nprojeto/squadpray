...
<template>
  <div class="max-w-2xl">
    ...
    <p class="text-sussurro mt-2">Quem entra no squad passa pelo aval de todos.</p>

    <AvisoErro :mensagem="erro" class="mt-6" />
    <AvisoErro :mensagem="aviso" tipo="ok" class="mt-6" />

    <p v-if="carregando" class="text-sussurro mt-8">Carregando…</p>

    <template v-else>
      <section class="mt-8">
        <p class="rotulo mb-3">Convites para você</p>
        <div v-if="!paraMim.length" class="painel p-6 text-sussurro text-sm">Nenhum convite no momento.</div>
        <div v-for="c in paraMim" :key="c.id" class="painel p-6 mb-3">
          ...
          <p v-if="c.squads?.objetivo" class="text-sussurro text-sm mt-2">{{ c.squads.objetivo }}</p>
          <p class="text-xs text-sussurro mt-2">
      ...
      <section class="mt-10">
        ...
        <div v-if="!paraAprovar.length" class="painel p-6 text-sussurro text-sm">Nada para aprovar agora.</div>
